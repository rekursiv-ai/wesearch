...
Installing the wheel places the trees at `<prefix>/share/rekursiv-ai-typeshed`
(`.venv/share/rekursiv-ai-typeshed` under uv), one per checker:
...
replacements in `rekursiv_ai_typeshed/typings/stdlib/` (`unittest/mock.pyi`,
...
- `basedpyright/` adds `stubs/` (basedpyright's third-party stubs, which ty
  ...
  `rekursiv_ai_typeshed/typings/basedpyright/`: `Intersection[A, B] = A`, since
