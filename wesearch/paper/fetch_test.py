...
class TestLooksLikePdf:
    ...
    def test_wrong_magic_rejected(self) -> None:
        assert not fetch.looks_like_pdf(b"<html>" + b"0" * 200)
    ...
    def test_custom_magic_shorter_than_default(self) -> None:
        ...
        assert fetch.looks_like_pdf(b"%PDF" + b"0" * 200, pdf_magic=b"%PDF")
...
class TestBatchOaUrls:
    ...
    def test_batch_failure_returns_none(self) -> None:
        with patch.object(s2, "batch", side_effect=RuntimeError("down")):
            assert fetch.batch_oa_urls(["DOI:1"]) is None


class TestDownload:
    def test_arxiv_success(self) -> None:
        with patch.object(fetch, "fetch", return_value=(_PDF, FetchSession())):
    ...
    def test_looked_up_none_skips_lookup(self) -> None:
        ...
        with (
            patch.object(s2, "get", s2_get),
            patch.object(fetch, "fetch", side_effect=[(_HTML, FetchSession())]),
            pytest.raises(NotFoundError),
        ):
            fetch.download("arxiv", "1706.03762", oa_url=None, oa_looked_up=True)
        s2_get.assert_not_called()

    def test_all_sources_fail_raises_not_found(self) -> None:
        with (
            patch.object(fetch, "fetch", side_effect=_fetch_error(404)),
            pytest.raises(NotFoundError),
        ):
            fetch.download("doi", "10.1/x", oa_url=None, oa_looked_up=True)
    ...
    def test_oa_download_failure_returns_none(self) -> None:
        ...
        with (
            patch.object(fetch, "fetch", side_effect=OSError("boom")),
            pytest.raises(NotFoundError),
        ):
            fetch.download("doi", "10.1/x", oa_url="http://oa/pdf", oa_looked_up=True)
    ...
    def test_s2_lookup_exception_returns_none(self) -> None:
        ...
        with (
            ...
            fetch.download("arxiv", "1706.03762")
